...
class ObjectManager extends Component with HasGameRef<DoodleDash> {
  ...
  final Map<String, bool> specialPlatforms = {
    ...
    'enemy': false, // level 5
    'supergirl': false, // level 3
    'spaceship': false, //level 5
  };
  ...
  void _maybeAddPowerup() {
    ...
    } else if (specialPlatforms['supergirl'] == true &&
        probGen.generateWithProbability(25)) {
      var supergirl = Supergirl(
        position: Vector2(_generateNextX(90), _generateNextY()),
      );
      add(supergirl);
      _powerups.add(supergirl);
    } else if (specialPlatforms['spaceship'] == true &&
        probGen.generateWithProbability(40)) {
      var spaceship = Spaceship(
        position: Vector2(_generateNextX(80), _generateNextY()),
      );
      add(spaceship);
      _powerups.add(spaceship);
    }
